...
<%
	...
	// 태그별로 속성, value 추출 가능
	// 언론사 이름은 소문자로 지정해주세요
	   int maximumPage = 10; //크롤링해올 총 페이지 개수
	...
	for(int i = 1; i <= maximumPage; i++){
		//사이트 url
        String URL = "http://www.ohmynews.com/NWS_Web/ArticlePage/Total_Article.aspx?PAGE_CD=N0120&pageno="+Integer.toString(i);
		//기사 관련 변수
        String title = "";
        String aTag = "";
        String updateDate = "";
        ...
        for(Element e: elem){
        	title = e.select("dt").select("[href]").text().replaceAll("\"", "");;   //기사 제목 a태그가 두개여서 난잡하게 처리
